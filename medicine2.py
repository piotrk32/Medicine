...
# Trenowanie modeli i obliczanie metryk dla każdego atrybutu oraz kumulatywnie
start_feature_count = 3  # Początkowa liczba cech
step = 2  # Krok przyrostu liczby cech

for i in range(start_feature_count, X_train.shape[1] + 1, step):
    knn = KNeighborsClassifier(metric='euclidean')
    knn.fit(X_train[:, :i], y_train)
    y_pred = knn.predict(X_test[:, :i])
    ...
    # Kumulatywne metryki
    if i == start_feature_count:
        cumulative_sensitivities.append(sensitivity)
        cumulative_specificities.append(specificity)
        cumulative_accuracies.append(accuracy)
    else:
        cumulative_sensitivities.append((cumulative_sensitivities[-1] * (i - step) + sensitivity) / i)
        cumulative_specificities.append((cumulative_specificities[-1] * (i - step) + specificity) / i)
        cumulative_accuracies.append((cumulative_accuracies[-1] * (i - step) + accuracy) / i)
